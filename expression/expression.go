...
var firstSet []grammarEntry = []grammarEntry{
	{symbol: EXPRESSION, derives: []uint8{OPEN_PARENTHESIS, LITERAL, NAME}},
	{symbol: EXPRESSION_LINE, derives: []uint8{ADD_OPERATOR, SUB_OPERATOR, EMPTY}},
	{symbol: TERM, derives: []uint8{OPEN_PARENTHESIS, LITERAL, NAME}},
	{symbol: TERM_LINE, derives: []uint8{TIMES_OPERATOR, DIV_OPERATOR, EMPTY}},
	{symbol: FACTOR, derives: []uint8{OPEN_PARENTHESIS, LITERAL, NAME}},
}
...
func expressionParser(tokenList []token) (Queue, error) {
	...
	var exprSyntax []syntaxTree = make([]syntaxTree, 0)
	var currentToken = 0

	exprSyntax = append(exprSyntax, syntaxTree{
	...
	for {
		...
		var treeLevel = len(exprSyntax) - 1

		for i, symbol := range exprSyntax[treeLevel].grammarExpasion {
			var derives []*grammarEntry = make([]*grammarEntry, 0)

			for _, entry := range expressionGrammar {
				if symbol == entry.symbol {
					derives = append(derives, &entry)
				}
			}
			if len(derives) == 0 {
				//	symbol is a terminal so, a token must be used
				if currentToken == i {
					exprSyntax[i].inputToken = &tokenList[currentToken]
					currentToken++
					break
				}
			...
			if len(derives) > 1 {
				//	use first and follow sets to guide the search
			}

			//	add a new level to the search tree
			exprSyntax = append(exprSyntax, syntaxTree{
				grammarExpasion: make([]uint8, 0),
				inputToken:      nil,
			})

			//	TODO: concat the derives + symbols to the right of var symbol
			terminalOnly = false
			break
		}
	...
	//	create the postfix from the syntax tree
	postfix := NewQueue()
